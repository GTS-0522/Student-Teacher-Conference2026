(() => {
  ...
  function postToApi(payload) {
    return new Promise((resolve, reject) => {
      ...
      const iframeName = `mendan-frame-${requestId}`;
      const iframe = document.createElement("iframe");
      const form = document.createElement("form");
      const input = document.createElement("input");
      const timer = setTimeout(() => finish(new Error("送信がタイムアウトしました。もう一度お試しください。")), 20000);
      ...
      function onMessage(event) {
        ...
        if (!data || data.source !== "mendan-api" || data.requestId !== requestId) return;
        if (data.ok) finish(null, data);
        else finish(new Error(data.message || "処理できませんでした。"));
      }
  ...
  function buildCalendar() {
    ...
    lastLabel.className = "time-label last";
    lastLabel.style.top = `${END_MINUTES - START_MINUTES}px`;
    lastLabel.textContent = "16:30";
    timeAxis.append(lastLabel);
