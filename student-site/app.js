(() => {
  ...
  const EVENTS_CACHE_KEY = "mendanEventsCacheV2";
  const LOAD_TIMEOUT_MS = 30000;
  const LOAD_RETRY_DELAYS = [0, 800, 2000];
  ...
  function storeCachedEvents(items) {
    try {
      sessionStorage.setItem(EVENTS_CACHE_KEY, JSON.stringify({ events: items, savedAt: Date.now() }));
    } catch (_) {
      // プライベートブラウズ等で保存できない場合も、通常表示は続けます。
    }
  }
  ...
  function jsonpEventsOnce() {
    return new Promise((resolve, reject) => {
      const callback = `mendanCallback_${Date.now()}_${Math.random().toString(36).slice(2)}`;
      const script = document.createElement("script");
      const timer = setTimeout(() => cleanup(new Error("予定の読み込みがタイムアウトしました。")), LOAD_TIMEOUT_MS);
  ...
  async function jsonpEvents() {
    let lastError;
    for (const delay of LOAD_RETRY_DELAYS) {
      if (delay) await sleep(delay);
      try {
        return await jsonpEventsOnce();
      } catch (error) {
        lastError = error;
      }
    }
    throw lastError || new Error("予定を読み込めませんでした。");
  }
  ...
  async function loadEvents(showFeedback = true) {
    ...
    if (isLoadingEvents) {
      if (showFeedback) showToast("予定を更新中です。少しお待ちください。");
      return;
    }
    ...
    if (!events.length) {
      ...
      if (cached) {
        ...
        if (updated) updated.textContent = "前回読み込んだ予定を表示中";
        buildCalendar();
      }
    }
    ...
    try {
      ...
      if (updated) updated.textContent = events.length ? "前回の予定を表示中（更新できませんでした）" : "予定を読み込めませんでした";
  ...
  function initStudentCalendar() {
    ...
    window.addEventListener("online", () => loadEvents(false));
  ...
  function initRequestForm() {
    ...
    if (localIsoDate() > REQUEST_DEADLINE) {
      showClosedMessage();
      return;
    }
